...
resize_img_train = zeros(resize_size, num_train);
resize_img_test = zeros(resize_size, num_test);
...
for i = 1:num_train
    ...
    img_v = img(:);
    resize_img_train(:, i) = img_v(1:resize_size);
...
for j = 1:num_test
    ...
    img = imresize(img, resize_ratio);
    img_v = img(:);
    resize_img_test(:, j) = img_v(1:resize_size);
    label = load(['Face Database/TestImages/', label_name]);
    label_test(j) = label(groupID);
end

all_img=[resize_img_train,resize_img_test];
...
net.divideParam.trainInd = 1:size(resize_img_train,2)*0.8;
net.divideParam.valInd = size(resize_img_train,2)*0.8+1:size(resize_img_train,2);
net.divideParam.testInd= size(resize_img_train,2)+1:size(all_img,2);
net.trainParam.lr=0.001;
net.trainParam.goal=0.001;
[net, tr]  = train(net, all_img, all_label);

%% anlysis
pred_train = net(resize_img_train(:,1:size(resize_img_train,2)*0.8));
pred_val = net(resize_img_train(:,size(resize_img_train,2)*0.8+1:size(resize_img_train,2)));
pred_test = net(resize_img_test);
perf_train = 1 - mean(abs(label_train(:,1:size(resize_img_train,2)*0.8)-pred_train));
perf_vaild =1-mean(abs(label_train(:,size(resize_img_train,2)*0.8+1:size(resize_img_train,2))-pred_val));
